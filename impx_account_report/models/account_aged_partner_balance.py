...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        journal_clause = ''
        if ctx.get('journal_ids'):
            journal_clause = ' AND l.journal_id IN %s '
            arg_list += (tuple(ctx['journal_ids']),)

        analytic_account_clause = ''
        if ctx.get('analytic_account_ids'):
            analytic_account_clause = ' AND l.analytic_account_id in %s '
            arg_list += (tuple(ctx['analytic_account_ids']._ids),)

        analytic_tag_clause = ''
        if ctx.get('analytic_tag_ids'):
            analytic_tag_clause = ' AND l.id in (SELECT account_move_line_id FROM account_analytic_tag_account_move_line_rel WHERE account_analytic_tag_id IN %s) '
            arg_list += (tuple(ctx['analytic_tag_ids']._ids),)
        ...
        for i in range(5):
            ...
            if journal_clause:
                args_list += (tuple(ctx['journal_ids']),)

            if analytic_account_clause:
                args_list += (tuple(ctx['analytic_account_ids']._ids),)

            if analytic_tag_clause:
                args_list += (tuple(ctx['analytic_tag_ids']._ids),)
        ...
        args_list = (tuple(move_state), tuple(account_type), date_from, tuple(partner_ids))
        if journal_clause:
            args_list += (tuple(ctx['journal_ids']),)
